...
params = StdioServerParameters(command="uv", args=["run", "python", "-m", "src.mcp.accounts_mcp_server"], env=None)
...
async def read_accounts_resource(name):
    async with stdio_client(params) as streams:
        async with mcp.ClientSession(*streams) as session:
            await session.initialize()
            result = await session.read_resource(f"accounts://accounts_mcp_server/{name}")
